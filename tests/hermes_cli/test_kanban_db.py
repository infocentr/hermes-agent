...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    # so opt into WAL to exercise the WAL-refusal fallback path this test covers.
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    kb._INITIALIZED_PATHS.clear()
    hermes_state_wal._wal_fallback_warned_paths.clear()
    # Local carry: DELETE is the kanban journal default — opt into WAL so this
    # WAL-silently-refused fallback test actually reaches apply_wal_with_fallback.
    monkeypatch.setenv("HERMES_KANBAN_JOURNAL", "wal")
