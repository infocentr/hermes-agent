...
def _apply_journal_policy(
    ...
    *,
    ...
    """Local carry (#54732): apply the kanban journal policy, return active mode.

    Honors HERMES_KANBAN_JOURNAL, else config ``kanban.journal_mode`` (read via
    the canonical loader — test_config_read_guard forbids a raw yaml.safe_load),
    defaulting to DELETE for the DELETE-mode watchdog + multi-service sharing.
    Falls back to WAL (with the helper's own network-FS DELETE fallback) otherwise.
    """
    import os
...
def _open_configured(path: Path, under_lock) -> tuple[sqlite3.Connection, Any]:
    ...
    try:
        ...
        with _INIT_LOCK:
            # Local carry (#54732): honor HERMES_KANBAN_JOURNAL / config
            # kanban.journal_mode, defaulting to DELETE journaling — the
            # DELETE-mode watchdog + multi-service sharing rely on it. Falls back
            # to WAL (with the helper's own network-FS DELETE fallback) otherwise.
            _apply_journal_policy(conn, db_label=f"kanban.db ({path.name})", default_delete=True)
