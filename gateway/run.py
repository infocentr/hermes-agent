...
def _select_cached_agent_history(
    ...
    # Local carry 0cfe6f60c: compare REPLAYABLE rows, not the raw in-memory list.
    # Gateway restore handoffs leave an empty user row in _session_messages that
    # _build_gateway_agent_history drops from the persisted replay, so counting it
    # creates a permanent false disk=N/memory=N+1 warning and replaces the cleaned
    # history with the dirty live copy. Preserve rich tool rows even when empty.
    replayable_live = [
        msg for msg in live_history
        if isinstance(msg, dict)
        and msg.get("role") not in {None, "", "system", "session_meta"}
        and (bool(msg.get("content")) or "tool_calls" in msg
             or "tool_call_id" in msg or msg.get("role") == "tool")
    ]
    if len(replayable_live) > len(persisted_history):
        # Upstream #50502 guard: only prefer live when it holds a real,
        # non-ephemeral UNPERSISTED row (a longer all-durable list is an expected
        # replay-filtering delta). The empty restore-handoff row is excluded by
        # the replayable filter above, so it can't satisfy this.
        has_unpersisted_row = any(
            not msg.get("_db_persisted") and not _is_ephemeral_scaffolding(msg)
            for msg in replayable_live)
