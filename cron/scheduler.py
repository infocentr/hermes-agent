...
# ── Local carries (cron guardrails) ────────────────────────────────────────
# Unfinished-work markers (#58160): a cron agent can produce a polished report
# while admitting validation/logging/revert/cleanup is still pending — that is
# NOT a successful unattended run, so it must not be marked ok.
...
# Report-start markers for autonomous agents whose model may prepend raw
# chain-of-thought as plain content; everything before the LAST marker is
# scratch-work, stripped from the DELIVERED message only (raw output is still
# saved verbatim). No marker → returned unchanged (no-op for other jobs).
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if provider_reachable and (
        # Local carry b7fa5ffb4: word-boundary on standalone "timeout" so a
        # compound token like "timeout-SIGKILL" in a job's own report text is
        # not misclassified as a provider timeout.
        "readtimeout" in lower or "timed out" in lower
...
def _resolve_cron_agent_setup(job: dict, job_id: str, job_name: str, jc) -> _CronAgentSetup:
    ...
    # Local carry: per-job max_turns override. _resolve_turn_limit above reads
    # config.yaml's max_turns, NOT the job's own field, so a job that sets
    # max_turns (e.g. system-improvement-agent → 180) would otherwise be ignored.
    # Let the job dict override the config default for long-running unattended jobs.
    _job_mt = job.get("max_turns") or job.get("max_iterations")
    if _job_mt:
        try:
            _job_mt = int(_job_mt)
            if _job_mt > 0:
                setup.max_iterations = _job_mt
        except (ValueError, TypeError):
            logger.warning("Job '%s': invalid max_turns %r, ignoring", job_id, _job_mt)
...
def run_job(
    ...
    try:
        ...
        # Local carry (#58160): a polished report that admits validation/revert/
        # cleanup is still pending is NOT a successful unattended run — fail it so
        # the operator is alerted instead of getting a false green tick.
        _incomplete = _cron_final_response_incomplete(final_response)
        if _incomplete:
            logger.warning("Job '%s': unfinished-work guardrail — %s", job_name, _incomplete)
            _audit.write(dict(result, response_silent=_is_cron_silence_response(final_response or "")), _incomplete)
            return False, output, final_response, _incomplete
...
def _compose_run_delivery(
    ...
    if blocked_config and not success:
        ...
        # Local carry: strip leaked reasoning-model scratch-work before the
        # agent's report marker (no-op when no marker present; raw output saved
        # verbatim elsewhere).
        deliver_content = _strip_reasoning_preamble(final_response)
    else:
