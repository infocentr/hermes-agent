...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    # ``timeout-SIGKILL`` (which appears in system-improvement reports) — local
    # carry b7fa5ffb4 (PR #58160): upstream's plain ``"timeout" in lower`` would
    # false-match that compound token, so keep the word-boundary regex.
    if provider_reachable and (
...
def run_job(
    ...
    *,
    ...
    try:
        ...
        # use a larger budget without changing global defaults (local carry b7fa5ffb44).
...
def _run_one_job_body(
    ...
    *,
    ...
    try:
        ...
        # (local carry b7fa5ffb44 / PR #58160)
        if success:
