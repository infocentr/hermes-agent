...
# Local carry (#57305): raw-PID kill detection. The word-based
# _GATEWAY_LIFECYCLE_PATTERN cannot catch `kill -TERM <pid>` with a literal
# numeric PID — no hermes/gateway token appears (real incident: an in-gateway
# agent, blocked four times by the pattern guard, killed the gateway by literal
# PID and cost a 419-message session). At terminal-execution time the gateway
# knows its own PID, so a literal-token match is exact and cannot false-positive
# on prose or on kills of unrelated processes. Dynamic forms (`kill $PID`,
# `xargs kill`) are out of scope — this is a foot-gun guard, not a boundary.
...
def command_kills_pid(text: str, pid: int) -> bool:
    """Return True if *text* has a kill/pkill invocation with *pid* as a literal token."""
