...
def gateway_lifecycle_block(
    *,
    ...
    # Local carry (#57305): an exact raw-PID kill of the gateway's own process
    # runs FIRST, before the heuristic word-pattern / launchctl checks below —
    # `kill -TERM <gateway-pid>` carries no hermes/gateway token to match.
    if command_kills_pid(command, os.getpid()):
        return _blocked_json(
            f"Blocked: refusing to kill the gateway's own process (PID {os.getpid()}) "
            "from inside it — the service would not come back.",
            "error",
        )
